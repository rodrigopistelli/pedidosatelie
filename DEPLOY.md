...
2. New Project → nome `projeto01`, região próxima (ex: US East).
...
2. New → **Blueprint** → selecione o repo `projeto01` (usa o `render.yaml` da raiz).
...
4. Deploy. Anote a URL, ex: `https://projeto01-api.onrender.com`.
   Teste: `https://projeto01-api.onrender.com/api/health` → `{"ok":true}`.

## 3. Frontend — Vercel (5 min)

1. Conta em https://vercel.com (login com GitHub).
2. Add New → Project → importe `projeto01`.
...
5. Deploy. Anote a URL, ex: `https://projeto01.vercel.app`.

## 4. Amarrar CORS (2 min)

1. No Render → serviço da API → Environment → `FRONTEND_URL` =
   `https://projeto01.vercel.app` → Save (faz redeploy sozinho).
